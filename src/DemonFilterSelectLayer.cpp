...
class $modify(GrdDemonFilterSelectLayer, DemonFilterSelectLayer) {
    ...
    bool init() {
        ...
        for (unsigned int i = 0; i < this->getChildrenCount(); ++i) {
            if (auto newObj = dynamic_cast<CCLayer*>(this->getChildren()->objectAtIndex(i))) {
                layer = newObj;
            }
        }
        ...
        for (unsigned int i = 0; i < layer->getChildrenCount(); ++i) {
            auto obj = layer->getChildren()->objectAtIndex(i);
        ...
        for (unsigned int i = 0; i < menu->getChildrenCount(); ++i) {
            if (auto newObj = dynamic_cast<CCMenuItemSpriteExtra*>(menu->getChildren()->objectAtIndex(i))) {
